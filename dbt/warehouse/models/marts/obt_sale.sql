...
select
    ...
    {{ dbt_utils.star(from=ref('dim_film'), relation_alias='dim_film', except=["film_key", "last_update"]) }},
    {{ dbt_utils.star(from=ref('dim_customer'), relation_alias='dim_customer', except=["customer_key", "last_update", "customer_address_id"]) }},
    {{ dbt_utils.star(from=ref('dim_address'), relation_alias='dim_address', except=["address_key", "last_update"]) }},
    {{ dbt_utils.star(from=ref('dim_staff'), relation_alias='dim_staff', except=["staff_key", "last_update"]) }},
    {{ dbt_utils.star(from=ref('dim_store'), relation_alias='dim_store', except=["store_key", "last_update"]) }},
    {{ dbt_utils.star(from=ref('dim_inventory'), relation_alias='dim_inventory', except=["inventory_key", "last_update", "film_title"]) }},
